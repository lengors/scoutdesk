...
/**
 * Base class for conditional converters from {@link ConstraintViolation} to {@link HttpStatus}. The converter matches
 * if the root bean type of the constraint violation is assignable from the requested type.
 *
 * @param <T> the type of the root bean of the constraint violation
 * @author lengors
 */
public abstract class AbstractConstraintViolationHttpStatusConditionalConverter<T>
  implements Converter<ConstraintViolation<T>, HttpStatus>, ConditionalConverter {
   ...
   */
  protected AbstractConstraintViolationHttpStatusConditionalConverter(
